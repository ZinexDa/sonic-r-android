...
void ReplayVoice_Load(int clipIndex)
{
    ...
    for (int i = 0; i < info->pieceCount; i++) {
        ...
        if (g_soundBuffers[REPLAY_VOICE_SLOT(i)] != NULL) {
            loaded++;
        }
    }

    if (loaded == info->pieceCount) {
        s_clipIndex = clipIndex;
        s_pieceCount = info->pieceCount;
        return;
    }

    /* Retail fallback: if split pieces aren't present, try unsplit REPLAY<N>.WAV */
    char retailName[16];
    snprintf(retailName, sizeof(retailName), "REPLAY%d.WAV", clipIndex + 1);
    LoadSoundEffect(retailName, REPLAY_VOICE_SLOT_FIRST);
    if (g_soundBuffers[REPLAY_VOICE_SLOT_FIRST] != NULL) {
        s_clipIndex = clipIndex;
        s_pieceCount = 1;
    }
}
...
void ReplayVoice_Start(void)
{
    ...
    for (int i = 0; i < s_pieceCount; i++) {
        ...
        unsigned int dur = 0;
        if (s_pieceCount == 1) {
            dur = (unsigned int)SFX_ClipDurationMs(REPLAY_VOICE_SLOT(i));
        }
        if (dur == 0) {
            dur = PieceDurationMs(s_clipIndex, i);
        }
        at += dur;
    }
    s_endMs = at;
