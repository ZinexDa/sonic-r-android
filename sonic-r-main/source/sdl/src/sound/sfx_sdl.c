...
static float s_masterSfxVolume = 1.0f;

void SFX_SetMasterVolume(float vol)
{
    if (vol < 0.0f) vol = 0.0f;
    if (vol > 1.0f) vol = 1.0f;
    s_masterSfxVolume = vol;
    g_optSfxVolume = (vol > 0.001f) ? 8 : 0;
    if (s_ready) {
        SetAllSoundVolumes();
        if (vol <= 0.001f) {
            Mix_Volume(-1, 0);
...
float SFX_GetMasterVolume(void)
{
    return s_masterSfxVolume;
}
...
static int LoadWAVIntoSlot(int slot, const char *filename)
{
    ...
    if (chunk == NULL) {
        /* Fallback: try uppercase and lowercase if the passed filename casing didn't match */
        char altFilename[64];
        strncpy(altFilename, filename, sizeof(altFilename) - 1);
        altFilename[sizeof(altFilename) - 1] = '\0';
        for (char *p = altFilename; *p; p++) {
            *p = (*p >= 'a' && *p <= 'z') ? (char)(*p - 'a' + 'A') : *p;
        }
        snprintf(path, sizeof(path), DATA_DIR "/SOUND/SFX/%s", altFilename);
...
void InitDirectSound(void)
{
    ...
    if (g_optSfxVolume <= 1 && s_masterSfxVolume > 0.001f) {
        g_optSfxVolume = 8;
    }
    SetAllSoundVolumes();
}
...
void SFX_Play(int slot, int loop, int freq)
{
    ...
    float vol = sfx_ds_linear(g_masterVolume) * s_masterSfxVolume;
    if (IS_REPLAY_VOICE_SLOT(slot)) {
        vol = 1.0f * s_masterSfxVolume;   /* binary exempts the announcer slot from attenuation */
    }
    ...
    DebugLog("SFX_Play: slot 0x%02X, vol=%d\n", slot, mixVol);
}
...
void SFX_SetVolume(int slot, int dsVolume)
{
    ...
    float linear = sfx_ds_linear(dsVolume) * sfx_slot_trim(slot) * s_masterSfxVolume;
